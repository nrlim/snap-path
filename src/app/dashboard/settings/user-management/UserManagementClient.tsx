  ...
  return (
    <div className="space-y-6 pb-10">
      <div>
        ...
        <h1 className="mt-2 text-3xl font-light tracking-tight text-foreground">User Management</h1>
        <p className="mt-1 max-w-2xl text-sm text-muted-foreground">{scope === "client" ? "Kelola user dalam client team Anda." : "Kelola role user internal SnapPath dan relasi user ke client."}</p>
      </div>

      <section className="rounded-lg border border-border bg-card shadow-sm">
        <div className="border-b border-border px-5 py-4">
          <h2 className="text-base font-medium text-foreground">Daftar User</h2>
          <p className="mt-1 text-sm text-muted-foreground">Table standar dengan search, filter, sorting, dan pagination.</p>
        </div>

        <div className="grid grid-cols-1 gap-3 border-b border-border p-4 lg:grid-cols-[1fr_180px_220px_120px]">
          <TableSearch value={search} onChange={(value) => { setSearch(value); setPage(1); }} placeholder="Search name, email, role, client..." />
          <select value={roleFilter} onChange={(event) => { setRoleFilter(event.target.value); setPage(1); }} className="rounded-md border border-border bg-card px-3 py-2.5 text-base text-foreground sm:text-sm">
            <option value="all">Semua role</option>
            {roles.map((role) => <option key={role} value={role}>{role}</option>)}
          </select>
          <select value={clientFilter} onChange={(event) => { setClientFilter(event.target.value); setPage(1); }} className="rounded-md border border-border bg-card px-3 py-2.5 text-base text-foreground sm:text-sm">
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[860px] text-left text-sm">
            ...
            <tbody className="divide-y divide-border/60">
              {paginatedUsers.length === 0 ? <tr><td colSpan={5} className="px-4 py-10 text-center text-muted-foreground">Tidak ada user yang sesuai filter.</td></tr> : paginatedUsers.map((user) => (
                <tr key={user.id} className="hover:bg-muted/50">
                  <td className="px-4 py-3"><p className="font-medium text-foreground">{userName(user)}</p></td>
                  <td className="px-4 py-3 text-muted-foreground">{user.email}</td>
                  <td className="px-4 py-3"><span className="rounded-md bg-primary/10 px-2.5 py-1 text-xs font-medium text-primary">{user.role}</span></td>
                  <td className="px-4 py-3 text-muted-foreground">{user.client?.name || "Tidak terikat"}</td>
                  <td className="px-4 py-3">
                    <form action={submit} className="flex justify-end gap-2">
                      <input type="hidden" name="userId" value={user.id} />
                      <select name="role" defaultValue={editableRoles.includes(user.role) ? user.role : editableRoles[0]} className="rounded-md border border-border bg-card px-2 py-2 text-sm text-foreground">{editableRoles.map((role) => <option key={role} value={role}>{role}</option>)}</select>
                      <select name="clientId" defaultValue={user.clientId || ""} className="rounded-md border border-border bg-card px-2 py-2 text-sm text-foreground">{scope === "platform" && <option value="">No Client</option>}{clients.map((client) => <option key={client.id} value={client.id}>{client.name}</option>)}</select>
                      <button disabled={isPending} className="rounded-lg border border-border px-3 py-2 text-sm font-medium text-muted-foreground hover:bg-muted disabled:opacity-50">Update</button>
                    </form>
                  </td>
                </tr>
              ))}
