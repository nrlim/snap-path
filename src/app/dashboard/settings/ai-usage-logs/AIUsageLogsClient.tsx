    ...
    <div className="space-y-6 pb-10">
      <section className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        {summaryCards.map((item) => (
          <div key={item.clientName} className="rounded-lg border border-border bg-card p-4 shadow-sm">
            <p className="text-xs font-medium uppercase tracking-[0.1em] text-muted-foreground">{item.clientName}</p>
            <p className="mt-3 text-2xl font-extrabold text-foreground">{formatNumber(item.totalTokens)}</p>
            <p className="mt-1 text-xs text-muted-foreground">{formatNumber(item.requests)} request AI bulan ini</p>
            ...
            <div className="mt-3 rounded-md bg-muted/40 px-3 py-2 text-xs font-medium text-foreground">Estimasi layanan: {formatUsd(item.costUsd)} / {formatIdr(item.costUsd)}</div>
          </div>
        ))}
      </section>

      <section className="rounded-lg border border-border bg-card shadow-sm">
        <div className="border-b border-border px-5 py-4"><h2 className="text-base font-medium text-foreground">Estimasi Pemakaian per Job Clinical Pathway</h2><p className="mt-1 text-sm text-muted-foreground">Menggabungkan seluruh call dalam satu jobId agar estimasi pemakaian per workflow mudah diaudit.</p></div>
        <div className="grid grid-cols-1 gap-3 border-b border-border p-4 lg:grid-cols-[1fr_220px_120px]">
          <TableSearch value={jobSearch} onChange={(value) => { setJobSearch(value); setJobPage(1); }} placeholder="Search client atau job ID..." />
          <select value={jobClientFilter} onChange={(event) => { setJobClientFilter(event.target.value); setJobPage(1); }} className="rounded-md border border-border bg-card px-3 py-2.5 text-base text-foreground sm:text-sm"><option value="all">Semua client</option>{clients.map((client) => <option key={client} value={client}>{client}</option>)}</select>
          <select value={jobPageSize} onChange={(event) => { setJobPageSize(Number(event.target.value)); setJobPage(1); }} className="rounded-md border border-border bg-card px-3 py-2.5 text-base text-foreground sm:text-sm">{defaultPageSizes.map((size) => <option key={size} value={size}>{size} / page</option>)}</select>
        </div>
        <div className="overflow-x-auto"><table className="w-full min-w-[920px] text-left text-sm"><thead className="bg-muted/40 text-xs uppercase tracking-wider text-muted-foreground"><tr><th className="px-4 py-3"><SortButton field="lastRequestAt" label="Last Request" sortField={jobSortField} sortDirection={jobSortDirection} onSort={handleJobSort} /></th><th className="px-4 py-3"><SortButton field="clientName" label="Client" sortField={jobSortField} sortDirection={jobSortDirection} onSort={handleJobSort} /></th><th className="px-4 py-3"><SortButton field="jobId" label="Job ID" sortField={jobSortField} sortDirection={jobSortDirection} onSort={handleJobSort} /></th><th className="px-4 py-3 text-right"><SortButton field="requests" label="AI Calls" sortField={jobSortField} sortDirection={jobSortDirection} onSort={handleJobSort} /></th><th className="px-4 py-3 text-right"><SortButton field="inputTokens" label="Input" sortField={jobSortField} sortDirection={jobSortDirection} onSort={handleJobSort} /></th><th className="px-4 py-3 text-right"><SortButton field="outputTokens" label="Output" sortField={jobSortField} sortDirection={jobSortDirection} onSort={handleJobSort} /></th><th className="px-4 py-3 text-right"><SortButton field="totalTokens" label="Total" sortField={jobSortField} sortDirection={jobSortDirection} onSort={handleJobSort} /></th><th className="px-4 py-3 text-right"><SortButton field="costUsd" label="Est. Usage" sortField={jobSortField} sortDirection={jobSortDirection} onSort={handleJobSort} /></th></tr></thead><tbody className="divide-y divide-border/60">{paginatedJobs.length === 0 ? <tr><td colSpan={8} className="px-4 py-10 text-center text-muted-foreground">Tidak ada job yang sesuai filter.</td></tr> : paginatedJobs.map((job) => <tr key={job.jobId} className="hover:bg-muted/50"><td className="px-4 py-3 text-xs text-muted-foreground">{formatDate(job.lastRequestAt)}</td><td className="px-4 py-3 font-medium text-foreground">{job.clientName}</td><td className="px-4 py-3 font-mono text-xs text-muted-foreground">{job.jobId}</td><td className="px-4 py-3 text-right font-mono">{formatNumber(job.requests)}</td><td className="px-4 py-3 text-right font-mono">{formatNumber(job.inputTokens)}</td><td className="px-4 py-3 text-right font-mono">{formatNumber(job.outputTokens)}</td><td className="px-4 py-3 text-right font-mono font-medium text-foreground">{formatNumber(job.totalTokens)}</td><td className="px-4 py-3 text-right font-mono"><div className="font-medium text-foreground">{formatUsd(job.costUsd)}</div><div className="mt-0.5 text-xs font-medium text-muted-foreground">{formatIdr(job.costUsd)}</div></td></tr>)}</tbody></table></div>
