...
type AIUsageLog = { id: string; clientName: string; jobId: string | null; endpoint: string; aiProvider: string | null; aiModel: string | null; inputTokens: number; outputTokens: number; totalTokens: number; durationMs: number; createdAt: string };
...
export default function AIUsageLogsClient({ summaryCards, jobCosts, logs }: { summaryCards: SummaryCard[]; jobCosts: JobCost[]; logs: AIUsageLog[] }) {
  ...
  const filteredLogs = useMemo(() => {
    ...
    return logs
      .filter((log) => {
        const matchesSearch = !query || [log.clientName, log.endpoint, log.aiModel || "", log.jobId || ""].some((value) => value.toLowerCase().includes(query));
        const matchesClient = logClientFilter === "all" || log.clientName === logClientFilter;
        const matchesModel = modelFilter === "all" || (log.aiModel || "default") === modelFilter;
        return matchesSearch && matchesClient && matchesModel;
      })
      .sort((a, b) => {
        const aValue = logSortField === "createdAt" ? new Date(a.createdAt).getTime() : logSortField === "costUsd" ? estimateCostUsd(a) : (a[logSortField] ?? "");
        const bValue = logSortField === "createdAt" ? new Date(b.createdAt).getTime() : logSortField === "costUsd" ? estimateCostUsd(b) : (b[logSortField] ?? "");
        return compareValues(aValue, bValue, logSortDirection);
      });
  }, [logClientFilter, logSearch, logSortDirection, logSortField, logs, modelFilter]);
  ...
  return (
    <div className="space-y-6 pb-10">
      <section className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        {summaryCards.map((item) => (
          <div key={item.clientName} className="rounded-lg border border-border/80 bg-surface p-4 shadow-sm">
            ...
            <p className="mt-1 text-xs text-text-subtle">{formatNumber(item.requests)} AI request bulan ini</p>
            ...
            <div className="mt-3 rounded-md bg-surface-elevated/50 px-3 py-2 text-xs font-semibold text-text">Estimasi: {formatUsd(item.costUsd)} / {formatIdr(item.costUsd * USD_TO_IDR)}</div>
          </div>
        ))}
      </section>

      <section className="rounded-lg border border-border/80 bg-surface shadow-sm">
        <div className="border-b border-border/60 px-5 py-4"><h2 className="text-base font-bold text-text">Estimasi Biaya per Job Clinical Pathway</h2><p className="mt-1 text-sm text-text-subtle">Menggabungkan semua AI call dalam satu jobId agar terlihat estimasi biaya per request/workflow.</p></div>
      ...
      <section className="rounded-lg border border-border/80 bg-surface shadow-sm">
        <div className="border-b border-border/60 px-5 py-4"><h2 className="text-base font-bold text-text">Log Request AI</h2><p className="mt-1 text-sm text-text-subtle">Detail setiap call AI: endpoint internal AI, provider, model, token consumption, dan estimasi biaya.</p></div>
        <div className="grid grid-cols-1 gap-3 border-b border-border/60 p-4 lg:grid-cols-[1fr_220px_180px_120px]"><TableSearch value={logSearch} onChange={(value) => { setLogSearch(value); setLogPage(1); }} placeholder="Search endpoint, client, job ID, provider, model..." /><select value={logClientFilter} onChange={(event) => { setLogClientFilter(event.target.value); setLogPage(1); }} className="rounded-md border border-border bg-surface px-3 py-2.5 text-base text-text sm:text-sm"><option value="all">Semua client</option>{clients.map((client) => <option key={client} value={client}>{client}</option>)}</select><select value={modelFilter} onChange={(event) => { setModelFilter(event.target.value); setLogPage(1); }} className="rounded-md border border-border bg-surface px-3 py-2.5 text-base text-text sm:text-sm"><option value="all">Semua model</option>{models.map((model) => <option key={model} value={model}>{model}</option>)}</select><select value={logPageSize} onChange={(event) => { setLogPageSize(Number(event.target.value)); setLogPage(1); }} className="rounded-md border border-border bg-surface px-3 py-2.5 text-base text-text sm:text-sm">{defaultPageSizes.map((size) => <option key={size} value={size}>{size} / page</option>)}</select></div>
        <div className="overflow-x-auto"><table className="w-full min-w-[1100px] text-left text-sm"><thead className="bg-surface-elevated/50 text-xs uppercase tracking-wider text-text-subtle"><tr><th className="px-4 py-3"><SortButton field="createdAt" label="Waktu" sortField={logSortField} sortDirection={logSortDirection} onSort={handleLogSort} /></th><th className="px-4 py-3"><SortButton field="clientName" label="Client" sortField={logSortField} sortDirection={logSortDirection} onSort={handleLogSort} /></th><th className="px-4 py-3"><SortButton field="endpoint" label="AI Operation" sortField={logSortField} sortDirection={logSortDirection} onSort={handleLogSort} /></th><th className="px-4 py-3 text-text-subtle">Provider</th><th className="px-4 py-3"><SortButton field="aiModel" label="Model" sortField={logSortField} sortDirection={logSortDirection} onSort={handleLogSort} /></th><th className="px-4 py-3 text-right"><SortButton field="inputTokens" label="Input" sortField={logSortField} sortDirection={logSortDirection} onSort={handleLogSort} /></th><th className="px-4 py-3 text-right"><SortButton field="outputTokens" label="Output" sortField={logSortField} sortDirection={logSortDirection} onSort={handleLogSort} /></th><th className="px-4 py-3 text-right"><SortButton field="totalTokens" label="Total" sortField={logSortField} sortDirection={logSortDirection} onSort={handleLogSort} /></th><th className="px-4 py-3 text-right"><SortButton field="costUsd" label="Est. Cost" sortField={logSortField} sortDirection={logSortDirection} onSort={handleLogSort} /></th><th className="px-4 py-3 text-right"><SortButton field="durationMs" label="Durasi" sortField={logSortField} sortDirection={logSortDirection} onSort={handleLogSort} /></th></tr></thead><tbody className="divide-y divide-border/60">{paginatedLogs.length === 0 ? <tr><td colSpan={10} className="px-4 py-10 text-center text-text-subtle">Tidak ada log yang sesuai filter.</td></tr> : paginatedLogs.map((log) => { const costUsd = estimateCostUsd(log); return <tr key={log.id} className="hover:bg-surface-elevated/30"><td className="px-4 py-3 text-xs text-text-subtle">{formatDate(log.createdAt)}</td><td className="px-4 py-3 font-medium text-text">{log.clientName}</td><td className="px-4 py-3 text-xs text-text-subtle">{log.endpoint}</td><td className="px-4 py-3 text-text-subtle">{log.aiProvider || "-"}</td><td className="px-4 py-3 text-text-subtle">{log.aiModel || "default"}</td><td className="px-4 py-3 text-right font-mono">{formatNumber(log.inputTokens)}</td><td className="px-4 py-3 text-right font-mono">{formatNumber(log.outputTokens)}</td><td className="px-4 py-3 text-right font-mono font-bold text-text">{formatNumber(log.totalTokens)}</td><td className="px-4 py-3 text-right font-mono font-bold text-text">{formatUsd(costUsd)}</td><td className="px-4 py-3 text-right text-text-subtle">{log.durationMs}ms</td></tr>; })}</tbody></table></div>
