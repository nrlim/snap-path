  ...
  return (
    <div className="space-y-6">
      <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <div className="rounded-xl border border-border/80 bg-surface p-5 shadow-sm">
          <p className="text-xs font-bold uppercase tracking-wider text-text-faint">Total request aktif</p>
          <p className="mt-2 text-2xl font-bold text-primary">{formatRequest(clients.reduce((sum, client) => sum + client.requestBalance, 0))}</p>
        </div>
        <div className="rounded-xl border border-border/80 bg-surface p-5 shadow-sm">
          <p className="text-xs font-bold uppercase tracking-wider text-text-faint">Aturan penggunaan</p>
          <p className="mt-2 text-sm leading-6 text-text-subtle">Kuota request berkurang 1 setiap validasi Clinical Pathway atau AI mapping yang dijalankan client. Credit USD tetap dicatat sebagai biaya internal super admin.</p>
        </div>
      </section>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[980px] text-left text-sm">
            ...
            <tbody className="divide-y divide-border/60">
              {paginatedClients.length === 0 ? <tr><td colSpan={canTopUp ? 7 : 6} className="px-4 py-10 text-center text-text-subtle">Tidak ada client.</td></tr> : paginatedClients.map((client) => {
                const latest = client.requestLedgers[0];
                return (
                  <tr key={client.id} className="hover:bg-surface-elevated/30">
                    <td className="px-4 py-3"><p className="font-bold text-text">{client.name}</p><p className="text-xs text-text-subtle">{client.requestLedgers.length} transaksi request terbaru tersimpan</p></td>
                    ...
                    <td className="px-4 py-3 text-right text-xl font-bold tabular-nums text-primary">{formatRequest(client.requestBalance)}</td>
                    <td className="px-4 py-3 text-right font-mono text-xs text-text-subtle">${formatCredit(client.creditBalance)}</td>
                    <td className="px-4 py-3 text-text-subtle">{latest ? <div><p className={latest.amount > 0 ? "font-semibold text-green-700" : "font-semibold text-orange-700"}>{latest.amount > 0 ? "+" : ""}{formatRequest(latest.amount)} request · {latest.type}</p><p className="mt-0.5 text-xs">{formatDate(latest.createdAt)}</p></div> : "Belum ada transaksi"}</td>
      ...
      {topUpClient && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4 backdrop-blur-sm sm:items-center">
          <form action={submitTopUp} className="w-full max-w-lg overflow-hidden rounded-[24px] border border-border/80 bg-surface shadow-xl">
            <div className="flex items-center justify-between border-b border-border/60 px-5 py-4"><div><h3 className="text-base font-bold text-text">Top up request</h3><p className="text-sm text-text-subtle">Client: {topUpClient.name}</p></div><button type="button" onClick={() => setTopUpClient(null)} className="rounded-md p-2 text-text-subtle hover:bg-surface-elevated"><X className="h-4 w-4" /></button></div>
            <div className="grid gap-4 p-5">
              <input type="hidden" name="clientId" value={topUpClient.id} />
              <label className="text-sm font-medium text-text">Jumlah request<input name="amount" type="number" min="1" step="1" required className="mt-2 block w-full rounded-md border border-border bg-surface px-3 py-2.5 text-base text-text sm:text-sm" placeholder="100" /></label>
              <label className="text-sm font-medium text-text">Catatan<input name="description" className="mt-2 block w-full rounded-md border border-border bg-surface px-3 py-2.5 text-base text-text sm:text-sm" placeholder="Pembelian paket request manual" /></label>
            </div>
            <div className="flex justify-end gap-3 border-t border-border/60 px-5 py-4"><button type="button" onClick={() => setTopUpClient(null)} className="rounded-lg border border-border px-4 py-2.5 text-sm font-medium text-text-subtle hover:bg-surface-elevated">Batal</button><button disabled={isPending} className="rounded-lg bg-primary px-4 py-2.5 text-sm font-medium text-white disabled:opacity-50">Tambah request</button></div>
          </form>
        </div>
      )}
    </div>
