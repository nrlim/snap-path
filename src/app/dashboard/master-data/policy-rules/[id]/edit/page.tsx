...
export default async function EditPolicyRulePage(props: {
  ...
  // Enforce RBAC
  if (user.role === 'CLIENT_ADMIN' && rule.clientId !== user.clientId) {
    notFound();
  }
    ...
    <div className="w-full space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between border-b border-border pb-4">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-light tracking-tight text-foreground">Edit Policy Rule</h1>
          <p className="text-sm text-muted-foreground font-light mt-2 max-w-2xl leading-6">
            Ubah parameter aturan polis {rule.ruleCode}. Perubahan akan segera memengaruhi validasi klaim berikutnya.
          </p>
        </div>
        <Link
